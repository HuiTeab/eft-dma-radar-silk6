...
namespace eft_dma_radar.Silk6.Misc.Input;
...
internal static class HotkeyManager
{
    ...
    [
        ...
        new("ToggleEspWindow", "Toggle ESP Window", "ESP",
            ...
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.ESP.EspWindow.Toggle(); }),

        new("EspCycleRenderMode", "Cycle ESP Render Mode", "ESP",
            "Cycle player render mode: None → Bones → Box → HeadDot",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.ESP.EspWindow.CycleRenderMode(); }),
        ...
        new("ToggleSidebar", "Toggle Sidebar", "Layout",
            ...
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.ToggleVisibility(); }),
        ...
        new("SidebarSlot1", "Sidebar: Players", "Layout",
            ...
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.Toggle(0); }),

        new("SidebarSlot2", "Sidebar: Loot", "Layout",
            "Toggle the Loot widget (sidebar slot 2)",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.Toggle(1); }),

        new("SidebarSlot3", "Sidebar: Aimview", "Layout",
            "Toggle the Aimview widget (sidebar slot 3)",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.Toggle(2); }),

        new("SidebarSlot4", "Sidebar: Quests", "Layout",
            "Toggle the Quests panel (sidebar slot 4)",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.Toggle(3); }),

        new("SidebarSlot5", "Sidebar: Settings", "Layout",
            "Toggle the Settings panel (sidebar slot 5)",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.Sidebar.Toggle(4); }),

        // Aimview PiP
        new("AimviewCycleSize", "Cycle Aimview Size", "Widgets",
            "Cycle aimview PiP size: Floating → Small → Medium → Large",
            static e => { if (e.IsDown) { var c = SilkProgram.Config; c.AimviewPipSize = (c.AimviewPipSize + 1) % 4; c.MarkDirty(); eft_dma_radar.Silk6.UI.Widgets.AimviewWidget.RequestPipResnap(); } }),

        new("AimviewCycleCorner", "Cycle Aimview Corner", "Widgets",
            "Cycle aimview PiP corner: TL → TR → BR → BL",
            static e => { if (e.IsDown) { var c = SilkProgram.Config; c.AimviewPipCorner = (c.AimviewPipCorner + 1) % 4; c.MarkDirty(); eft_dma_radar.Silk6.UI.Widgets.AimviewWidget.RequestPipResnap(); } }),

        new("ResetSidePanels", "Reset Side Panels", "Layout",
            "Snap Players / Loot / Quests back into the right dock layout",
            static e => { if (e.IsDown) eft_dma_radar.Silk6.UI.Shell.RightDock.RequestResnap(); }),
