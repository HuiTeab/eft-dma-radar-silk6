...
namespace eft_dma_radar.Silk6.UI.Panels
{
    internal static partial class SettingsPanel
    {
        ...
        private static void DrawEspTab()
        {
            ...
            // ── Window state ──
            bool open = eft_dma_radar.Silk6.UI.ESP.EspWindow.IsOpen;
            if (UIControls.ToggleRow("ESP Window Open", ref open))
            {
                eft_dma_radar.Silk6.UI.ESP.EspWindow.Toggle();
                Config.ShowEspWidget = eft_dma_radar.Silk6.UI.ESP.EspWindow.IsOpen;
                Config.MarkDirty();
            }
                ...
                tooltip: "Render rate of the ESP window (0 = unlimited).\nIndependent of the radar FPS."))
            {
                ...
                eft_dma_radar.Silk6.UI.ESP.EspWindow.ApplyTargetFps();
                Config.MarkDirty();
            }
            ...
            if (eft_dma_radar.Silk6.UI.ESP.EspWindow.IsOpen)
            {
                ImGui.SameLine();
                if (ImGui.SmallButton("Move ESP to Monitor"))
                    eft_dma_radar.Silk6.UI.ESP.EspWindow.ApplyTargetMonitor();
            }
