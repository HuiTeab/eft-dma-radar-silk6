...
namespace eft_dma_radar.Silk6.UI.Panels
{
    internal static partial class SettingsPanel
    {
        private static async Task ToggleWebRadarAsync(bool enable)
        {
            try
            {
                if (enable)
                {
                    await eft_dma_radar.Silk6.Web.WebRadarServer.StartAsync(
                        Config.WebRadarPort,
                ...
                {
                    await eft_dma_radar.Silk6.Web.WebRadarServer.StopAsync();
                }
            }
            catch (Exception ex)
            {
        ...
        private static void DrawGeneralTab()
        {
...

            {
                ...
                if (ImGui.Button("\u2728 Show Welcome Tour"))
                    eft_dma_radar.Silk6.UI.Shell.FirstRunTour.Open();
                if (ImGui.IsItemHovered())
            ...
            if (Config.WebRadarEnabled)
            {
                ...
                if (eft_dma_radar.Silk6.Web.WebRadarServer.IsRunning)
                {
                    ...
                    // Private address
                    ImGui.Spacing();
                    var privateAddr = eft_dma_radar.Silk6.Web.WebRadarServer.PrivateAddress;
                    ...
                    // Public address
                    var publicAddr = eft_dma_radar.Silk6.Web.WebRadarServer.PublicAddress;
