...
namespace eft_dma_radar.Silk6.Config
{
    /// <summary>
    ...
    /// Loaded from / saved to a JSON file in %AppData%\eft-dma-radar-silk6\.
    /// </summary>
    public sealed class SilkConfig
    {
        private static readonly string _configDir =
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "eft-dma-radar-silk6");
        ...
        // see-through / force-blocker rule edits survive restart. Defaults
        // mirror the Arena defaults — the layer mask is conservative
        // (only layer 16 = player colliders) to avoid silently disabling
        // vischeck if the user forgets to tune it.
