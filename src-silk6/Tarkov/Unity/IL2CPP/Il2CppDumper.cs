    ...
    public static partial class Il2CppDumper
    {
        ...
        public static void Dump()
        {
            ...
            try
            {
                ...
                tableOk = eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(tablePtr);
                if (!tableOk)
            ...
            foreach (var sc in schema)
            {
                ...
                if (sc.TypeIndex.HasValue)
                {
                    ...
                    if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(klassPtr))
                    {
                ...
                else if (sc.ResolveViaChild is not null)
                {
                    ...
                    for (int depth = 0; depth < MaxParentDepth && eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(walkPtr); depth++)
                    {
                        ulong parentPtr = ReadPtr(walkPtr + Offsets.Il2CppClass.Parent);
                        if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(parentPtr))
                            break;
        ...
        private static List<(string Name, string Namespace, ulong KlassPtr, int Index)> ReadAllClassesFromTable(ulong tablePtr)
        {
            ...
            for (int offset = 0; offset < MaxClasses; offset += chunkSize)
            {
                ...
                {
                    if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(chunk[i]))
                        hasValid = true;
                }
            ...
            {
                if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(ptrs[i]))
                    validIndices.Add(i);
            }
            ...
            for (int j = 0; j < validIndices.Count; j++)
            {
                ...
                if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(p.NamePtr))
                {
                    nameEntries[j] = ScatterReadEntry<UTF8String>.Get(p.NamePtr, MaxNameLen);
                    stringBatch.Add(nameEntries[j]);
                }

                if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(p.NamespacePtr))
                {
        ...
        private static Dictionary<string, int> ReadClassFields(ulong klassPtr)
        {
            ...
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(fieldsBase)) return result;
            ...
            {
                if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(rawFields[i].NamePtr))
                {
        ...
        private static Dictionary<string, ulong> ReadClassMethods(ulong klassPtr, ulong gaBase)
        {
            ...
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(methodsBase)) return result;
            ...
            {
                if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(methodPtrs[i])) continue;
            ...
            for (int i = 0; i < methodPtrs.Length; i++)
            {
                ...
                if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(info.MethodPointer) || info.MethodPointer < gaBase) continue;
                if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(info.NamePtr)) continue;
        ...
        public static void DumpClassFields(ulong objectAddress, string? label = null)
        {
            try
            {
                if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(objectAddress))
                {
                ...
                while (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(currentKlass) && depth < MaxDepth)
                {
                    depth++;
        ...
        private static void DumpSingleClassFieldsWithValues(ulong klassPtr, ulong objectAddress)
        {
            ...
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(fieldsBase))
            {
            ...
            for (int i = 0; i < rawFields.Length; i++)
            {
                ...
                if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(rawFields[i].TypePtr))
                {
        ...
        private static string ReadStringFieldValue(ulong addr)
        {
            ...
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(ptr)) return $"<bad ptr 0x{ptr:X}>";
            try
            {
        ...
        private static string ReadPointerOrValueFieldValue(ulong addr)
        {
            ...
            if (raw <= uint.MaxValue)
                return $"{(int)(uint)raw}";
            if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(raw))
                return $"0x{raw:X}";
            return $"0x{raw:X}";
        }
        ...
        {
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(addr)) return 0;
        ...
        {
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(addr)) return null;
