...
namespace eft_dma_radar.Silk6.Tarkov.Unity.IL2CPP
{
    public static partial class Il2CppDumper
    {
            ...
            string fallback)
        {
            ...
            if (eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(data) && ptrToName.TryGetValue(data, out var byPtr))
                return byPtr;
        ...
        public static void DumpAll()
        {
            ...
            if (!eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(tablePtr))
            {
            ...
            List<(string Name, string Namespace, ulong KlassPtr, int Index)> classes)
        {
            ...
            for (int depth = 0; depth < MaxParentDepth && active.Count > 0 && unresolvedGenericCount > 0; depth++)
            {
                ...
                for (int chunkStart = 0; chunkStart < active.Count; chunkStart += ScatterChunkSize)
                {
                    ...
                    for (int j = 0; j < chunkLen; j++)
                    {
                        if (!typed[j].IsFailed && eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(typed[j].Result))
                ...
                for (int chunkStart = 0; chunkStart < validParentIndices.Count; chunkStart += ScatterChunkSize)
                {
                    ...
                    for (int k = 0; k < chunkLen; k++)
                    {
                        if (!typed[k].IsFailed && eft_dma_radar.Silk6.Misc.Utils.IsValidVirtualAddress(typed[k].Result))
