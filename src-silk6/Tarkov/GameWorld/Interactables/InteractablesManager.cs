...
namespace eft_dma_radar.Silk6.Tarkov.GameWorld.Interactables
{
    /// <summary>
    ...
    internal sealed class InteractablesManager
    {
        ...
        private static List<Door> ReadDoorsBatched(List<ulong> doorPtrs)
        {
            ...
            // Components[0] (Transform) → TransformInternal
            // Unity 6: compArr+0x08 IS the native TransformInternal directly.
            // (Unity 2022 needed two extra hops: +Comp_ObjectClass +0x10. On Unity 6 those
            //  land on Il2CppClass metadata, which produces garbage transforms.)
            ...
            using (var s4 = Memory.GetScatter(VmmFlags.NOCACHE))
            {
                for (int i = 0; i < count; i++)
                {
                    if (components[i].IsValidVirtualAddress())
                        s4.PrepareReadValue<ulong>(components[i] + 0x08); // First component = native TransformInternal
                }
                s4.Execute();
                ...
                {
                    if (components[i].IsValidVirtualAddress())
                        s4.ReadValue<ulong>(components[i] + 0x08, out tis[i]);
                }
            }
