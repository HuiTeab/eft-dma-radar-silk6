...
namespace eft_dma_radar.Silk6.Tarkov.GameWorld.Exits
{
    /// <summary>
    ...
    internal sealed class Exfil
    {
        ...
        /// <summary>
        /// Reads world position from the exfil's transform.
        /// Unity 6 native scene chain (4 hops — no managed-wrapper at hop 5):
        ///   baseAddr → +0x10 (native MonoBehaviour)
        ///            → +Comp_GameObject(0x38) (native GameObject)
        ///            → +GO_Components(0x38) (ComponentArray)
        ///            → +0x08 (native TransformInternal — directly, no Comp_ObjectClass hop).
        /// </summary>
        private static Vector3 ReadPosition(ulong baseAddr)
        {
            try
            {
                if (!Memory.TryReadPtr(baseAddr + 0x10, out var nativeMB, false) || !nativeMB.IsValidVirtualAddress())
                    return Vector3.Zero;
                if (!Memory.TryReadPtr(nativeMB + UnityOffsets.Comp_GameObject, out var nativeGo, false) || !nativeGo.IsValidVirtualAddress())
                    return Vector3.Zero;
                if (!Memory.TryReadPtr(nativeGo + UnityOffsets.GO_Components, out var compArr, false) || !compArr.IsValidVirtualAddress())
                    return Vector3.Zero;
                if (!Memory.TryReadPtr(compArr + 0x08, out var transformInternal, false) || !transformInternal.IsValidVirtualAddress())
                    return Vector3.Zero;
