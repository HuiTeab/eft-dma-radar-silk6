...
using eft_dma_radar.Silk6.Tarkov.GameWorld.Exits;
using eft_dma_radar.Silk6.Tarkov.GameWorld.Loot;
using eft_dma_radar.Silk6.Tarkov.GameWorld.Player;
using eft_dma_radar.Silk6.Tarkov.Unity.Collections;
                ...
                if (game.Explosives is not null)
                {
                    foreach (var ex in game.Explosives.Snapshot)
                    {
                        try
                        {
                            ...
                            {
                                Name = ex is eft_dma_radar.Silk6.Tarkov.GameWorld.Explosives.Grenade g2 ? g2.Name : ex.GetType().Name,
            ...
            try
            {
                ...
                if (game.Explosives?.Snapshot is { } grenades && grenades.Count > 0)
                {
                    ...
                    foreach (var ex in grenades)
                    {
                        try
                        {
                            if (ex is not eft_dma_radar.Silk6.Tarkov.GameWorld.Explosives.Grenade g) continue;
