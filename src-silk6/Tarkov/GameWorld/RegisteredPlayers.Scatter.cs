...
using static eft_dma_radar.Silk6.Tarkov.Unity.UnityOffsets;

namespace eft_dma_radar.Silk6.Tarkov.GameWorld
{
    internal sealed partial class RegisteredPlayers
    {
        ...
        private static void TryInitTransform(ulong playerBase, PlayerEntry entry)
        {
            ...
            // Unity 6: lookTransformPtr may already be the native TransformAccess (Path A)
            // OR it may be a managed-wrapper that requires one extra +0x10 deref (Path B).
            // Per UNITY_ENGINE_CHANGES.md §6 — probe by reading +HierarchyOffset(0x40).
            // If it's a valid pointer, candidate is already native. Otherwise treat it
            // as a managed wrapper and deref +0x10 to get the native TransformAccess.
            ulong transformInternal;
            ulong taHierarchy;
            if (Memory.TryReadPtr(lookTransformPtr + TransformAccess.HierarchyOffset, out var probeHier, false)
                && probeHier.IsValidVirtualAddress())
            {
                // Path A: lookTransformPtr is already native TransformAccess
                transformInternal = lookTransformPtr;
                taHierarchy = probeHier;
            }
            else
            {
                // Path B: managed wrapper — deref +0x10 to inner native TransformAccess
                if (!Memory.TryReadPtr(lookTransformPtr + 0x10, out transformInternal, false)
                ...
                if (!Memory.TryReadPtr(transformInternal + TransformAccess.HierarchyOffset, out taHierarchy, false)
