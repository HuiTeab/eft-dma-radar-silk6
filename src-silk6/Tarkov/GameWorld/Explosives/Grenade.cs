...
namespace eft_dma_radar.Silk6.Tarkov.GameWorld.Explosives
{
    /// <summary>
    ...
    internal sealed class Grenade : IExplosiveItem
    {
        ...
        public Grenade(ulong baseAddr, ConcurrentDictionary<ulong, IExplosiveItem> parent)
        {
            ...
            // Read and cache transform hierarchy via the short 4-hop native scene chain.
            // Same chain as BTR / LootManager (confirmed working on Unity 6).
            // The 6-hop TransformChain (via Comp_ObjectClass +0x28 → +0x10) fails for scene objects
            // because the sub-Transform has no managed wrapper at hop 5.
            // Chain: baseAddr → +0x10 → +Comp_GameObject(0x38) → +GO_Components(0x38) → +0x08 = TI
            var nativeMB = Memory.ReadPtr(baseAddr + 0x10, false);
            var nativeGo = Memory.ReadPtr(nativeMB + Comp_GameObject, false);
            var compArr  = Memory.ReadPtr(nativeGo + GO_Components, false);
            var ti       = Memory.ReadPtr(compArr + 0x08, false);
