...
namespace eft_dma_radar.Silk6.DMA.ScatterAPI
{
    ...
    public sealed class ScatterReadEntry<T>() : IScatterEntry, IPooledObject<ScatterReadEntry<T>>
    {
        ...
        private void Configure(ulong address, int cb)
        {
            ...
            if (cb == 0 && _isValueType)
                cb = eft_dma_radar.Silk6.Misc.SizeChecker<T>.Size;
            CB = cb;
        }
        ...
        {
            int cb = eft_dma_radar.Silk6.Misc.SizeChecker<T>.Size;
...
#pragma warning restore CS8500
        ...
        private void SetClassResult(VmmScatter scatter)
        {
            ...
            {
                int size = eft_dma_radar.Silk6.Misc.SizeChecker<MemPointer>.Size;
            ...
            else if (this is ScatterReadEntry<eft_dma_radar.Silk6.Misc.UnicodeString> r3)
            {
                ...
                var nullIdx = eft_dma_radar.Silk6.Misc.Extensions.FindUtf16NullTerminatorIndex(ro);
                r3._result = nullIdx >= 0
                    ? Encoding.Unicode.GetString(buf[..nullIdx])
                    : Encoding.Unicode.GetString(buf);
            }
            else if (this is ScatterReadEntry<eft_dma_radar.Silk6.Misc.UTF8String> r4)
            {
                Span<byte> buf = CB > 0x1000 ? new byte[CB] : stackalloc byte[CB];
                buf.Clear();
                if (!scatter.ReadSpan(Address, buf)) { IsFailed = true; return; }
